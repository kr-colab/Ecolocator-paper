...
(e.g., GWAS, $F_{ST}$-outlier tests,
and genotype--environment association methods)
add resolution but struggle with polygenic traits,
correlated predictors, and complex demography
\citep{beaumont_evaluating_1997, hirschhorn_genome-wide_2005,
coop_using_2010, joost_spatial_2007}; Rellstab et al. 2015.
These limitations highlight the need for scalable approaches
that capture multilocus signatures of selection
while accounting for both geographic structure
and environmental gradients.

Recent advances in machine learning (ML) help address this gap.
ML methods excel at extracting patterns from high-dimensional data,
...
can predict geographic origin from genotypes alone
by leveraging multilocus patterns that elude classical statistics.
Other recent work incorporates ecological context more explicitly:
gradient forest methods link genomic variation
to environmental predictors to forecast climate-driven maladaptation
[Lotterhos et al., 2021],
...
disentangling adaptive signals from demographic confounders,
laying the groundwork for ML-based approaches
to local adaptation [Lotterhos and Whitlock, 2015].
Building on these ideas,
supervised ML has emerged as a powerful paradigm
in population genetics for capturing nonlinear associations
across thousands of loci and environmental variables
\citep{schrider_supervised_2018}.
ML models can detect genetic signals of local adaptation
even when neutral demographic processes or population structure
create background patterns
that confound traditional approaches.
...
Downstream feature attribution methods such as SHAP [CITE]
